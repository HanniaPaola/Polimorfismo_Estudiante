...
public class MenuController {
    ...
    void lista(MouseEvent event) {
        if (lista.getSelectionModel().getSelectedIndex() != -1) {
            Student selectedStudent = getStudentFromListview();
    ...
    void btn_agregar(MouseEvent event) {
        ...
        String apellido = tex_apellido.getText();
        if (!name.isEmpty() || !apellido.isEmpty()) {
            Student nuevo = new Student(name, apellido);
    ...
    private Student getStudentFromListview() {
        ...
        private Student parseEstudianteFromString(String selectedItem) {
        String[] parts = selectedItem.split(",");
        String name = parts[0];
        String apellido = parts[1];
        return new Student(name, apellido);
    }

    @FXML
    public void init(Stage stageRoot) {
        universidad = new Universidad(new BasedeDato1(), new BasedeDato2(), new BasedeDato3());
    }
}